...
// Token dikirim via query param terenkripsi dari widget SE
...
export default async function handler(req, res) {
  ...
  // Coba fetch dengan access token saat ini
  let result = await fetchCurrentlyPlaying(access);

  // Jika 401, refresh dulu lalu coba lagi
  if (result.status === 401) {
    ...
    // Kirim token baru agar widget bisa update URL-nya
